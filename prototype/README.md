...
Система включает два независимых модуля для работы с двухслойным графом:

### Модуль обработки графа (`graph-processor.js`)

Обрабатывает DAG граф с двумя слоями связей:
- **Слой 1**: Семантическая похожесть между постами (через embedding векторы)
- **Слой 2**: Общие NER (акторы) между постами

**Основные возможности:**
- Динамическая кластеризация вокруг выбранной точки
- Декластеризация (от полного коллапса до полного разбора)
- Кластеризация по одному слою или обоим одновременно
- Настраиваемые пороги через константы

### Модуль отображения графа (`graph-renderer.js`)

Визуализирует граф и обеспечивает интерактивность:
- Отображение двухслойного графа с разными цветами для каждого слоя
- Интерактивная кластеризация/декластеризация
- Физическая симуляция графа
- Коммуникация с модулем обработки

### Использование

1. **Демонстрация**: Откройте `graph-demo.html` для интерактивной демонстрации
2. **Интеграция**: Используйте `graph-example.js` для интеграции в существующий код
3. **Документация**: См. `docs/graph-modules.md` для подробной документации
...
processor.initializeGraph(posts, actors, relationships);

const renderer = new GraphRenderer('graphContainer', processor);
renderer.loadGraph(processor.exportToJSON());

// Кластеризация вокруг узла
